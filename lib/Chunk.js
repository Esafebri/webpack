...
class Chunk {
	...
	getAllParentsSet() {
		...
		this.parents.forEach(function addParent(parent) {
			if(parentsSet.has(parent)) return;
			parentsSet.add(parent);
			parent.parents.forEach(addParent);
		});
		return parentsSet;
	}
	...
	getAllChildrenSet() {
		...
		this.chunks.forEach(function addChild(parent) {
			if(childrenSet.has(parent)) return;
			childrenSet.add(parent);
			parent.chunks.forEach(addChild);
		});
		return childrenSet;
	}
	...
	getInitialParents() {
		...
		function visitChunk(chunk) {
			if(visited.has(chunk)) return;
			visited.add(chunk);
			if(chunk.isInitial()) {
				initial.add(chunk);
			} else {
				chunk.parents.forEach(visitChunk);
			}
		}
		visitChunk(this);
		return initial;
